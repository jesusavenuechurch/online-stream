...
<head>
    ...
    <style>
        body { 
            ...
            background: radial-gradient(circle at top center, #1D4069 0%, #0f172a 100%); 
        }
        .glass-card { 
        ...
        .input-dark:focus {
            border-color: #F07F22;
            outline: none;
            box-shadow: 0 0 0 3px rgba(240, 127, 34, 0.2);
        }
        .tab-active { 
            background: #1D4069; 
            color: white; 
            box-shadow: 0 4px 12px rgba(29, 64, 105, 0.4);
        }
...
<body class="text-slate-200 min-h-screen flex items-center justify-center p-6">
    ...
    <div class="w-full max-w-lg">
        ...
        <div class="flex justify-center mb-10">
            <div class="relative">
                <div class="absolute -inset-8 bg-[#F07F22]/10 rounded-full blur-3xl"></div>
                <img src="{{ asset('images/ventiq-noback.png') }}" 
                     alt="VENTIQ" 
                     class="relative h-20 w-auto object-contain drop-shadow-[0_0_15px_rgba(240,127,34,0.3)]">
            </div>
        </div>
        ...
        <div class="glass-card rounded-[2.5rem] p-8 md:p-10">
            ...
            <div id="registration-success" class="hidden text-center py-6 fade-in">
                ...
                <p class="text-slate-400 mb-8">Thank you for joining. You can now sign in to view the broadcast.</p>
                <button onclick="switchTab('login')" class="block w-full bg-[#1D4069] hover:bg-[#1D4069]/80 text-white py-4 rounded-xl font-bold transition text-center">Back to Sign In</button>
            </div>

            <div id="auth-content">
                <div class="text-center mb-8">
                    <h1 class="text-2xl font-bold text-white mb-1 uppercase tracking-tight">{{ $event->title }}</h1>
                    <p class="text-slate-400 text-sm">Please sign in to access the live stream</p>
                </div>
                ...
                <div id="form-login" class="space-y-6 fade-in">
                    <div>
                        <input type="text" id="login-username" class="input-dark w-full px-5 py-4 rounded-xl text-center text-lg font-bold" placeholder="Enter Username">
                    </div>
                    <div id="login-error" class="hidden text-red-400 text-sm text-center bg-red-400/10 py-2 rounded-lg border border-red-400/20"></div>
                    <button onclick="login()" id="login-btn" class="w-full bg-[#F07F22] hover:bg-[#F07F22]/90 text-white py-4 rounded-xl font-bold text-lg transition-all transform active:scale-[0.98]">Join Live Experience</button>
                    <p class="text-center text-sm text-slate-400">First time? <button onclick="switchTab('register')" class="text-[#F07F22] hover:underline font-bold">Create an account</button></p>
                </div>
                ...
                <div id="form-register" class="hidden space-y-4 fade-in">
                    ...
                    <div id="reg-error" class="hidden text-red-400 text-sm text-center bg-red-400/10 py-2 rounded-lg border border-red-400/20"></div>
                    <button onclick="register()" id="reg-btn" class="w-full bg-[#1D4069] hover:bg-[#1D4069]/90 text-white py-4 rounded-xl font-bold transition-all transform active:scale-[0.98]">Register Now</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    async function login() {
        ...
        try {
            // SYNCED: Now matches Route::post('/watch/{event}/authenticate')
            ...
            if (response.ok && data.success) {
                ...
                errEl.classList.remove('hidden');
                btn.disabled = false;
                btn.textContent = "Join Live Experience";
            }
        } catch (e) {
            errEl.textContent = "Server connection failed. Try again.";
            errEl.classList.remove('hidden');
            btn.disabled = false;
            btn.textContent = "Join Live Experience";
        }
    ...
    async function register() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                errEl.textContent = data.message || "An error occurred during registration";
                errEl.classList.remove('hidden');
                btn.disabled = false;
